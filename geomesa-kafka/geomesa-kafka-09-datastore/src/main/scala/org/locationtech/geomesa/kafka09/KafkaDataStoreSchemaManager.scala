...
trait KafkaDataStoreSchemaManager extends DataStore with LazyLogging {
  ...
  override def createSchema(featureType: SimpleFeatureType): Unit = {
    ...
    if(!AdminUtils.topicExists(zkUtils, kfc.topic)) {
      AdminUtils.createTopic(zkUtils, kfc.topic, partitions, replication)
    }
  ...
  override def getNames: util.List[Name] =
    zkUtils.getChildren(zkPath).map(name => new NameImpl(name) : Name).asJava
  ...
  override def removeSchema(typeName: String): Unit = {
    ...
    zkUtils.deletePathRecursive(getSchemaPath(typeName))

    // delete topic for "Streaming SFTs" but not for "Replay SFTs"
    fct.foreach { fc =>
      if (KafkaDataStoreHelper.isStreamingSFT(fc.sft) && AdminUtils.topicExists(zkUtils, fc.topic)) {
        AdminUtils.deleteTopic(zkUtils, fc.topic)
      }
    }
  }
  ...
  private def resolveTopicSchema(typeName: String): KafkaFeatureConfig = {
    ...
    val schema = zkUtils.readData(getSchemaPath(typeName))._1 //throws ZkNoNodeException if not found
    val topic = zkUtils.readData(getTopicPath(typeName))._1 // throws Zk...
    val sft = SimpleFeatureTypes.createType(typeName, schema)
    KafkaDataStoreHelper.insertTopic(sft, topic)
    val replay = Option(zkUtils.readData(getReplayConfigPath(typeName))._1)
  ...
  private val zkUtils = {
    val ret = ZkUtils(zookeepers, Int.MaxValue, Int.MaxValue, JaasUtils.isZkSecurityEnabled)
...
private[kafka09] case class KafkaFeatureConfig(sft: SimpleFeatureType) {
