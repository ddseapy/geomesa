...
class ConsumerRebalancer[K, V](consumer: KafkaConsumer[K, V], config: ConsumerConfig)
  ...
  private val zkClient = zkUtils.zkClient
  ...
  private def registerConsumerInZK(): Unit = {
    ...
    zkUtils.createEphemeralPathExpectConflict(path, consumerInfo)
  ...
  private def rebalance(cluster: Seq[Broker]): Boolean = {
    ...
    if (brokers.size == 0) {
      ...
      val assignmentContext = new AssignmentContext(config.groupId, consumerId, config.excludeInternalTopics, zkUtils)
      val partitionOwnershipDecision = partitionAssignor.assign(assignmentContext).head._2 //todo fix
